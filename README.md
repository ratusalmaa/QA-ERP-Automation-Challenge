...
it('Create Quotation', () => {
    ...
    cy.get('input#sale_promotion_id_0.o-autocomplete--input.o_input')
        .click()
        .type('Normal Monthly{downarrow}{enter}');
    
    // cy.get('ul.o-autocomplete--dropdown-menu')
    //     .should('be.visible')
    //     .find('li')
    //     .contains('Normal Monthly (3 Month)')
    //     .click();

    cy.get('input#sale_promotion_id_0.o-autocomplete--input.o_input')
        .should('have.value', 'Normal Monthly (3 Month)'); // Memastikan input sudah terisi
    ...
    cy.contains('Request').click();
    cy.get('tr.o_data_row').should('have.length.greaterThan', 0); // Memastikan ada baris Quotation yang tampil
...
it('First Approval', () => {
    cy.visit('/');
    cy.get('img.o_avatar.o_user_avatar.rounded').should('be.visible').click();
    cy.contains('Log out').click();

    cy.get('#login').type('jeanette.grace@mceasy.co.id');
    cy.get('#password').type('jeanette.grace@mceasy.co.id{enter}');
    cy.get('.o_home_menu').should('be.visible');
...
it('Second Approval', () => {
    cy.visit('/');
    cy.get('#login').type('yenny.widjojo@mceasy.co.id');
    cy.get('#password').type('yenny.widjojo@mceasy.co.id{enter}');
    cy.get('.o_home_menu').should('be.visible');
